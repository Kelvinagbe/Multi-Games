     // Import Firebase modules
        ...
        function generateReferralCode(uid) {
            ...
            const randomChars = uid.substring(0, 4) + Math.random().toString(36).substring(2, 6);
            return randomChars.toUpperCase();
        }
        ...
        function updateTransactionHistory(newTransactions) {
            ...
            // Limit to only the 3 most recent transactions
        ...
        async function updateProfileInfo(user) {
            try {
                ...
                // Update username
                const displayName = userData.displayName || user.displayName || 'GameMaster';
                if (usernameEl) {
                ...
                // Update avatar
                if (avatarEl) {
                    avatarEl.src = user.photoURL || userData.avatarURL || 'https://i.postimg.cc/gJZr2MPG/images-20.jpg';
                }
                ...
                if (referralCodeEl && shareReferralCodeEl) {
                    const code = userData.referralCode || generateReferralCode(user.uid);
                    referralCodeEl.textContent = code;
                    shareReferralCodeEl.value = code;
                    
                    // Save generated code if it doesn't exist
